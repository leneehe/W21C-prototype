class Dashboard::PlanController < ApplicationController
  ...
  def build_data_collection(symptom, unit, symptom_info)
    ...
    TrackedSymptom.where(user_id: current_user.id, symptom_id: symptom.id).each do |measurement|
      # collection_name =  ValueType.find(key).name
        @value_data.push({"x" => measurement.created_at, "y" => measurement.severity_score})
    end

    start_date = TrackedSymptom.all.sort_by { |x| x.created_at }.first.created_at
    end_date = TrackedSymptom.all.sort_by { |x| x.created_at }.last.created_at
